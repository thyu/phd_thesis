...
Various semi-supervised forest learning algorithms have been proposed. Shotton \etal \cite{Shotton2013} measured data compactness to relate labelled and unlabelled data points. Leistner \etal \cite{Leistner2009} designed a margin metric to evaluate with unlabelled data. On the other side, regression forest was widely adopted in body pose estimation, \eg \cite{Girshick2011, Sun2012}. In the proposed system, \STR\ forest adaptively combines the aforementioned semi-supervised and regression forest learning techniques in a single frame work.
...
% done 
The training dataset $\totalset$ is a combination of realistic data $\realset$ and synthetic data $\synset$ in equation \ref{eq/hand/dataset}. 
\begin{equation}
	\label{eq/hand/dataset} 
	\begin{aligned}
		\totalset 	& = \left\{ \realset, \synset \right\} = \left\{ \reallset, \realuset, \synset \right\} & \mbox{(all training data) } \\
		\clset 		& = \left\{ \realset, \synset \right\} & \mbox{(labelled training data) } \\ 
	\end{aligned}
\end{equation}
A small potion of $\realset$ is labelled, where the labelled and the remaining unlabelled subsets are defined as $\reallset$ and $\realuset$ respectively. All synthetic data $\synset$ are labelled with groundtruths. In addition, the collection of labelled data $\totalset$ is defined as $\clset$.  
...
The split function $\splitfunc$ of a node is a simple two-pixel test, similar to the proposed 3D HPE system in chapter \ref{chap/body}, as shown in equation \ref{eq/hand/pixeltest}. 
\begin{equation}
	\splitfunc(\patch) = \begin{cases} 
		1 & \patch(\textbf{i})-\patch(\textbf{j}) > \tau \\
		0 & \mbox{ Otherwise } \\
	\end{cases}
	\label{eq/hand/pixeltest}
\end{equation}
...
Since a large amount of training samples needs to be evaluated, reservoir sampling is employed to avoid memory restriction and speed up training as in \cite{Girshick2011}. The information gain $\entropy_{\viewlabel}(\clset)$ is defined in equation \ref{eq/hand/viewterm}.  
\begin{equation}
	\label{eq/hand/viewterm}
	\begin{aligned}
		\entropy_{\viewlabel}(\clset) & = 
		- \displaystyle\sum_{\viewlabel \in \viewlabelset} \frac{|\clset^{(\viewlabel)}|}{|\clset|}\log_{2}\left(\frac{|\clset^{(\viewlabel)}|}{|\clset|}\right)\\ 
		\viewterm & = 
		\entropy_{\viewlabel}(\clset) - 
	...
	\end{aligned}
\end{equation}
$\clset^{(\viewlabel)}$ denotes the subset of $\clset$ that is assigned view point label $\viewlabel$. Incoming training data $\clset$ are divided into subsets $\clset_{l}$ and $\clset_{r}$ to the left and right child nodes respectively.  
...
\begin{equation}
	\begin{aligned}	
		...
		- \displaystyle\sum_{\jointlabel \in \jointlabelset} \frac{|\clset^{(\jointlabel)}|}{|\clset|}\log_{2}\left(\frac{|\clset^{(\jointlabel)}|}{|\clset|}\right)\\ 
	\classterm & = 
	\entropy_{\jointlabel}(\clset) - 
	\frac{|\clset_{l}|}{|\clset|} \entropy_{\jointlabel}(\clset_{l}) -  
	\frac{|\clset_{r}|}{|\clset|} \entropy_{\jointlabel}(\clset_{r}) 
	\end{aligned}
	\label{eq/hand/classterm}
\end{equation}
...
Similar to Hough forest, each patch in $\clset$ is associated with a vote, which is a 3D vector from the patch's centroid to the center of a joint in $\jointlabelset$. Let $\votelabelset(\clset)$ be the voting vectors of current training data $\clset$, the regression term $\regterm$ is defined in equation \ref{eq/hand/regterm}.  
\begin{equation}
	\begin{aligned}
	\votelabelset(\clset) & = \{ \votelabel_{\patch}|\forall \patch \in \clset \}\\
	\trvar(\cdot) & = \mathrm{Tr}\left(\Sigma(\cdot)\right)\\ 
	\regterm & = \left[ 1 + 
...
Assuming appearances and poses are correlated under the same viewpoint, pose similarities between a pair of data points can be roughly estimated by the resemblance of their appearances. The unsupervised term $\usterm$ in equation \ref{eq/hand/usterm} evaluates the similarities among all realistic patches $\realset$ within a node.   
\begin{equation}
	\usterm = \left[ 1 + 
		\frac{|\crealset_{l}|}{|\crealset|} \trvar(\crealset_{l}) +  
	\frac{|\crealset_{r}|}{|\crealset|} \trvar(\crealset_{r}) \right]^{-1} 
	\label{eq/hand/usterm}
\end{equation}
...
The relationship between the sparse realistic data and dense synthetic data is learned via \emph{transductive learning}. Inspired from cross-modality boosting in \cite{Bronstein2010}, the transductive term $\pairterm$ preserves the cross-domain associations $\assoc$ as the training data pass down the trees: 
\begin{equation}
	\begin{split}
		\pairterm &= 
		\frac{|\{r,s\} \subset \clset_{l}| +  
		|\{r,s\} \subset \clset_{r}|}{|\{r,s\} \subset \clset|} \\   
...
Equations \ref{eq/hand/margin} and \ref{eq/hand/margin2} are responsible for the classification of viewpoint labels $\viewlabel$ and joint labels $\jointlabel$ in $\clset$ respectively. They measure the purity of a node with respect to viewpoint and patch label.
\begin{equation}
	\begin{array}{l@{}r}
		\viewparam  = 
		\begin{cases}
...
In addition, the free parameter $\tssparam$ controls the relative importance of transductive term $\pairterm$ to unsupervised term $\usterm$.  
...
% done 
Training data $\ksynset$ is first categorised with respect to their viewpoints in equation \ref{eq/hand/datadrivenclassify}. 
\begin{equation}
	\label{eq/hand/datadrivenclassify} 
...
\begin{equation}
	\begin{aligned}
		\GMM & = \{ \GMM_{1}, \dots, \GMM_{i}, \dots, \GMM_{|\viewlabelset|} \} \\ 
...
Patches are extracted densely given a testing depth image. They pass down the \STR\ forest to obtain their viewpoint $\testviewlabel$ and vote vectors $\testvotelabel$. Similar to Hough forest in \cite{Gall2011}, the patches vote for all $16$ joint locations according to $\testvotelabel$.  
...
A $\testonejoint_{\joint}$ is considered as a high-confidence joint when the Euclidean distance between $\hat{\mu}_{\joint}^{1}$ and $\hat{\mu}_{\joint}^{2}$ is smaller than a predefined threshold $\testqualthres$.  
For a high-confident $j$-th joint, the final output location $\testonejoint_{\joint}$ is represented by the mean of the dominating Gaussian in $\testGMM_{\joint}$. Otherwise, low-confidence joints are computed using the kinematic joint refinement algorithm in section \ref{sec/hand/kjr}. 
\begin{equation}
	\testonejoint_{\joint} = 
	\begin{cases}
		...
		\mbox{Joint refinement} & \mbox{ otherwise }  
	\end{cases}
	\label{eq/hand/highconf}
\end{equation}
...
Given the nearest Gaussian component $\{\mu_{\testviewlabel}^{nn}, \Sigma_{\testviewlabel}^{nn}\}$ of the high-confidence joints, each remaining low-confidence joint $\testonejoint_{\joint}$ are assigned in equation \ref{eq/hand/refine1}. 
\begin{equation}
		\{\tilde{\mu}, \tilde{\Sigma}\} 
		...
		\label{eq/hand/refine1}
\end{equation}
The output joint distribution $\{\tilde{\mu}, \tilde{\Sigma}\}$ is one of the Gaussian components in $\testGMM_{\joint}$ that is closer to the corresponding $\joint$-th joint location in $\{\mu_{\testviewlabel}^{nn}[\joint] \in \mathbb{R}^{3}, \Sigma_{\testviewlabel}^{nn}[\joint] \in \mathbb{R}^{3\times3}\}$ taken from $\{ \mu_{\testviewlabel}^{nn}, \Sigma_{\testviewlabel}^{nn} \} $. Finally, the refined output of a low-confidence joint $\testonejoint_{l}$ is computed by merging the Gaussians in equation \ref{eq:refine2} (\cf equation \ref{eq/body/combine}). 
\begin{equation}
	\testonejoint_{\joint} =
...
\begin{algorithm}
	...
	\ForEach{ Set of voting vectors for the $\joint$-th joint}{
		...
		\If{$||\hat{\mu}_{\joint}^{1} - \hat{\mu}_{\joint}^{2}||^2_2 < \testqualthres$}{
			...
			Compute the $\joint$-th joint location. (Equation \ref{eq/hand/highconf}) 
		}\Else{
	...
	Update the remaining low-confidence joint (Equation \ref{eq/hand/refine1} and \ref{eq:refine2}) \\
...
The proposed approach was evaluated under a single view scenario, comparing with the traditional regression forest algorithm in \cite{Gall2011} as a baseline. Since there was only one viewpoint in testing sequence $A$, $\viewterm$ in equation \ref{eq/hand/qualityfunction1} did not affect the experimental results. In other words, the proposed method and the baseline algorithm only differed in joint regression and classification, hence only $\classterm$,$\regterm$, $\pairterm$ and $\usterm$ were utilised in this experiment.    

Performances of algorithms were measured by their pixelwise classification accuracy per joint, similar to \cite{Shotton2011}. Figure \ref{fig/hand/single} shows the classification accuracy plots of the algorithm evaluated in this experiment.  
...
By coupling realistic data with synthetic data, the transductive term $\pairterm$ effectively learnt the discrepancies between the domains, which is important in recognising noisy and strongly occluded fingers.
...
Similar to \cite{Oikonomidis2011}, pose estimation performances were measured by joint localisation error.

\subsubsection{Quantitative results}

Figure \ref{fig/hand/multiquant} shows the average localisation errors of the testing sequences. It also shows similar error graphs from a stable joint (palm, $P$) and a difficult joint (index finger tip, $I3$). The proposed \STR\ forest, together with the data-driven kinematic joint refinement, outperformed FORTH in all aspects, especially for the finger tip joints that are noisy and frequently occluded. Since the proposed approach is frame-based, even though a few large estimation errors are observed, they were recovered quickly without drifting.    
...
In this experiment, the proposed joint refinement scheme increased the joint estimation accuracy in general, as illustrated in figure \ref{fig/hand/multiquant}. 
Some large classification errors, \eg figure \ref{fig/hand/multiquantc}, were fixed after applying joint refinement. It implied that the joint refinement process not only improved the accuracy of joint, but also avoided incorrect detections by validating the output of \STR\ forest with kinematic constraints.
