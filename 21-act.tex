...
Spatiotemporal textons, \ie retangular cuboids of voxels, are used as the input features. 
Figure \ref{fig/act/fastest} illustrates how the cuboids are extracted from the input videos using VFAST. 
Salient motions are detected by the VFAST detector using three axis-aligned circles of radius 3 pixels, \ie $r_{XY}, r_{YT}, r_{XT} = 3$, according to algorithm \ref{algo/eval/vfast}. 
Video cuboids are extracted from the VFAST interest points detected.
In this work, a video cuboid has dimensions of $5 \times 5$ voxels in space and $9$ voxels in time.
