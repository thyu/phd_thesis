...
\begin{figure}[ht]
	...
	\caption{\textbf{Joint classification accuracy of the single view sequence.} Classification accuracy is defined as the percentage of correct foreground pixels. The unsupervised term $\usterm$ equals to one in the ``Transductive only'' \STR\ forest, hence unlabelled data are not used in training. Data-driven joint refinement is not performed in the baseline and the \STR\ forest approaches.}
