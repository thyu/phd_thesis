...
Unlike sensor noise and depth errors \cite{Girshick2011, Baak2011}, these artifacts cannot be repaired or recovered easily. Consequently, a large discrepancy is observed between synthetic and realistic data.  
...
Hypotheses of 3D hand pose estimates from \STR\ forest are verified from this data-driven technique, which models the structure of human hands, without computing sophisicated inverse kinematics. 
...
Training a decriminative hand pose recogniser requires a large and extensive training dataset. Hand-labelling of hand pose data, however, is costly and ineffective due to occlusions and complicated articulations of human hands. 
...
% done 
On the other hand, discriminative methods learns a mapping from visual features to 3D hand pose articulations through classification or regression techniques. A discriminative hand pose estimator is constructed from a training dataset offline, which is usually based on holistic shape features \cite{Rosales2001, Athitsos2003, Romero2009, Wang2009, Keskin2012}. Discriminative approaches are usually more efficient than generative approaches as no tracking is performed. Additionally, many discriminative methods are frame-based, such that they can recognise hand poses from a single image. However, discimrinative approaches are restricted to a limited set of predefined poses, as they require a large labelled dataset for training, which is often very costly to obtain.  
...
Furthermore, their iterative optimisation processes are usually computationally expensive. Despite near real-time performance with latest GPU acceleration techniques \cite{Oikonomidis2011}, generative methods struggle to attain real-time performance in commodaity hardware. 
...
It has seen various successful applications \cite{Pan2010}, nontheless it has not been applied to articulated pose estimation. 
...
Futhermore, the distribution of labelled and unlabelled realistic data are described jointly in the proposed \STR\ forest using an unsupervised learning term. 
...
Hence, the kinematic datasest $\ksynset$ is not used to train the \STR\ dataset. 
% done 
...
\begin{figure}[ht]
	...
	\caption{\textbf{Color codes and labels of the 16 hand regions.}}
...
A joint was model as a 3D truncated Gaussian distribution centered at the joint location, while its variance was defined empirically according to the structure of human hands. Foreground pixels were clustered into one of these distributions and therefore assigned with labels $\jointlabel$. For example, the palm label usually contributed a larger region than the fingertip labels, due to the large variance in the palm joint.
...
Performances of candidate algorithms were measured by their pixelwise classification accuracy per joint \cite{Shotton2011}. Figure \ref{fig/hand/single} shows the classification accuracy plots of the algorithm evaluated in this experiment.  
...
Transductive learning term $\pairterm$ have improved the accuracy substantially, particularly for the finger tips which are less robust in the baseline algorithms.  
...
Nevertheless, semi-supervised learning significantly corrects those joints after transductive learning. To summarise, semi-supervised learning complements transductive learning by modeling the intermediate poses between the sparsely labelled realistic data.    
