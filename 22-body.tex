...
\begin{figure}[th]
	...
	% \caption{\textbf{Action detection in 3D pose estimation.} Action detection helps 3D pose estimation by providing the spatiotemporal structure of actions.} 
	\caption{\textbf{Action detection in 3D pose estimation.} Each atomic action has a unique spatiotemporal structure, \eg ``dance'' and ``clap''. Action detection determines the class label and the starting time of an atomic action. For example, a ``dance'' action is detected at time = $T-\Delta t$.  At current time = $T$, a strong pose prior is deduced from the spatiotemporal structure of the ``dance'' action.}
...
\begin{figure}[!th]
	...
	\caption{\textbf{Overview of the proposed framework.} A 2D deformable part model (DPM) is used for feature detection. Pose configurations are estimated globally by the action detectino forest, and locally by the regression forest. The final 3D pose is combined probabilistically using a late fusion scheme.}
...
The proposed 3D HPE framework is overviewed in figure \ref{fig/body/overview}. 
...
Given the classe labels $\action$ from action detection, the cross-modality regression forests $\forestr$ are applied to refine the 3D pose estimations in section \ref{sec/body/jrflearn}. 
...
The articulated model in figure \ref{fig/body/jointlabels} showns the $15$ joint labels defined in the proposed 3D HPE framework.
...
\centering
	...
	\caption{\textbf{Joint label names of the proposed 3D HPE system.} In this work, a 3D human pose is represented by fifteen 3d joint locations. }
...
\begin{sidewaysfigure}[!ht]
	...
	\caption{\textbf{2D joint localisation errors.} 2D localisation errors are the Euclidean distances between the back-projected joint coordinates and their corresponding ground truth locations. The proposed approaches are compared with the state-of-the-art 2D HPE algorithms.}
